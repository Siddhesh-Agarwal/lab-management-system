...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $systemcount = LabList::where("type", "Desktop")
            ->where("lab_name", Auth::user()->labname)
            ->count();
        $laptopCount = LabList::where("type", "Laptop")
            ->where("lab_name", Auth::user()->labname)
            ->count();
        $student = Student::where("isLoggedIn", 1)
            ->where("labname", $wow)
            ->count();
        // $systemcount = Lablist::where('lab_name', Auth::user()->labname)->count();
        $devicecount = Lab::where("lab_name", Auth::user()->labname)->sum(
            "count",
        );
    ...
    public function tables()
    {
        ...
        $labNames = Lab_Table::get();
        return view("admin.StudentTable", [
            "students" => $student,
            "labNames" => $labNames,
        ]);
    }
    ...
    {
        $logs = Student::where("isLoggedIn", 1)
            ->where("labname", Auth::user()->labname)
            ->get();
        ...
        foreach ($logs as $log) {
            $leaving = Logs::where("rollno", $log->rollno)
                ->latest()
                ->get()
                ->first();
    ...
    public function save_student(Request $request)
    {
        ...
        $main = StudentRecord::where(
            "regNo",
            "=",
            $request->input("rollno"),
        )->first();
        $res = Student::where("rollno", "=", $request->input("rollno"))
            ->latest()
            ->get()
            ->first();
        ...
        $lab = Lab_Table::where(
            "lab_name",
            "=",
            urldecode($request->labname),
        )->first();

        // Get the last allocated system number
        $lastAllocatedSystem = Student::where(
            "labname",
            "=",
            urldecode($request->labname),
        )
        ...
        $count = Lablist::where("lab_name", Auth::user()->labname)
            ->get()
            ->count();
        $limit = 0;
        // dd($count);
        ...
        if (is_null($res)) {
            ...
            $stud = Student::where("rollno", "=", $request->input("rollno"))
                ->latest()
                ->get()
                ->first();
            $stud->update(["isLoggedIn" => 1]);
            $message = sprintf(
                "Your system number is %s",
                $data["systemNumber"],
            );
            ...
            return redirect()
                ->action([AdminController::class, "index"])
                ->with("data_box", $data_box);
        } else {
            if ($res->isLoggedIn === 0) {
                $already_log = Student::where(
                    "rollno",
                    "=",
                    $request->input("rollno"),
                )
        ...
        $leaving = Logs::where("rollno", "=", $request->input("rollno"))
            ->latest()
            ->get()
            ->first();
        ...
        if ($isDeleted) {
            $message = sprintf(
                "Successfully Logged out ! worked time %d minutes",
                $timeDifference,
            );
        } else {
            $message =
                "Logged out, but could not remove student record or record not found.";
        }
