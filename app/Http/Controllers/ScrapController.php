...
class ScrapController extends Controller
{
    ...
    {
        try {
            $deleted = Scrap::where("id", "=", $id)->delete();
            if ($deleted) {
                return redirect()
                    ->back()
                    ->with("success", "Scrap data was deleted successfully !");
            }
            return redirect()
                ->back()
                ->with(
                    "error",
                    "Scrap record not found or could not be deleted !",
                );
        } catch (\Exception $e) {
    ...
    public function moveData($id)
    {
        try {
            ...
            if ($isDeleted) {
                return redirect()
                    ->route("scrap.index")
                    ->with("success", "Data moved to lab successfully !");
            }
            return redirect()
                ->route("scrap.index")
                ->with("error", "Failed to remove scrap record after moving !");
        } catch (\Exception $e) {
            return redirect()
                ->route("scrap.index")
